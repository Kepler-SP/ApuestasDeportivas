...
contract Apuesta {
    ...
    function agregarEquipos(string memory _team1, string memory _team2) public {
        arrayEncuentros.push(Encuentros(_team1, _team2, 0));
    }
    ...
    function apostar(
        ...
        if (usuario_cuenta[msg.sender] == 0x00) {
    ...
    function consultarResultado() public {
        Encuentros memory encuentro;
        for (uint i = 0; i < arrayEncuentros.length; i++) {
            encuentro = arrayEncuentros[i];
        }
